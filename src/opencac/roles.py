...
from .runtime import (
    ...
    RoutingConfig,
    _call_codex_exec,
    _call_planner_service,
    _call_research_service,
...
class Antigravity:
    ...
    def handle(self, instruction: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Try real Antigravity research service (JSON-RPC)
        research_url = self.inference.service_url("antigravity")
        if research_url:
            try:
                svc_payload = _call_research_service(research_url, query, instruction["session_id"])
                svc_payload.setdefault("query", query)
                svc_payload.setdefault("model_used", provider)
                svc_payload.setdefault("search_queries", [query])
                svc_payload.setdefault("stats", {})
                svc_payload["stats"]["research_service"] = research_url
                svc_payload.setdefault("sources", [])
                svc_payload.setdefault("findings", [])
                svc_payload.setdefault("research_assessment", {
                    "target": "user",
                    "summary": "Research performed by Antigravity service.",
                    "gaps": [],
                    "assumptions": [],
                })
                if callback_url:
                    svc_payload["callback_url"] = callback_url
                ...
                pass  # Fall through to local research
...
class ClaudeCodePlanner:
    ...
    def handle(self, report: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Try real Claude Bridge planner service
        planner_url = self.inference.service_url("claude-code")
        if planner_url:
            try:
                raw_text = _call_planner_service(planner_url, goal, context)
                plan_data = _parse_plan_json(raw_text)
                ai_steps = plan_data.get("steps", [])
                if ai_steps:
                    constraints = plan_data.get("constraints", ["Every hop must survive sidecar validation"])
                    acceptance = plan_data.get("acceptance_criteria", ["all steps complete successfully"])
                    plan_payload = {
                        "goal": goal,
                        "context": f"{context} | planner=claude-bridge",
                        "constraints": constraints,
                        "acceptance_criteria": acceptance,
                        "plan_assessment": {
                            "target": "researcher",
                            "summary": "Plan generated by Claude Bridge planner service.",
                            "gaps": [],
                            "assumptions": [],
                        },
                        "task": {
                        ...
                        "steps": ai_steps,
                        "planner_backend": {"endpoint": planner_url, "probe": "claude-bridge"},
                    }
                    if callback_url:
                ...
                pass  # Fall through to template plan
    ...
    def execute(self, plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for step in plan["payload"]["steps"]:
            ...
            if step["action"] == "create":
                ...
                codex = self.inference.codex_bin()
                if not codex:
                    result = {"step_id": step_id, "status": "failed", "output": "codex binary not configured", "files_changed": [], "error": "set OPENCAC_CODEX_BINARY"}
                else:
                    prompt = step.get("prompt") or step["description"]
                    codex_result = _call_codex_exec(codex, prompt, workspace=self.workspace)
                    if codex_result["exit_code"] == 0:
                        result = {"step_id": step_id, "status": "done", "output": codex_result["output"][:500], "files_changed": []}
                    else:
                        result = {"step_id": step_id, "status": "failed", "output": codex_result["output"][:500], "files_changed": [], "error": f"codex exit {codex_result['exit_code']}"}
